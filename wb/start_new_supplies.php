 ...
 ******************************************************************************************/
echo "<br> Формирование папок <br><br>";
$new_date = date('Y-m-d');
 ...
 if (!isset($new_real_arr_orders)) {
    ...
    // ecли этикеток нет, то снова делаем их запрос 
        for ($error_job = 0 ;$error_job < 10; $error_job++) {
            echo "<br> ---- $error_job **** НЕТ ЗАКАЗОв в ПОСТАВКЕ :".$supplyId['id']." *****<br>";
            usleep(500000);
            ...
            if (isset($new_real_arr_orders)) {
                echo "<br> ---- $error_job**** ПОЯВИЛИСЬ ЗАКАЗЫ в поставке :".$supplyId['id']." *****<br>";
                break;
...
if (count($new_real_arr_orders) < $count_order_art) { 
    ...
    for ($jj =0; $jj<10; $jj++) {
        echo "<br> --- Не Хватает заказов в Поставкe ;Цикл: $jj<br>";
            $real_temp_count = count($new_real_arr_orders);
            unset($new_real_arr_orders);
            unset($arr_real_orders_error);
            echo "<br> **** ~~~~ Не хватает Заказов в поставке ($real_temp_count), а должно быть ($count_order_art) *****<br>";
            usleep(500000);
            $arr_real_orders_error = get_orders_from_supply($token_wb, $supplyId['id']); // список Заказов которые ТОЧНО полпали в Поставку
     ...
     if (count($new_real_arr_orders) == $count_order_art) { 
        $real_temp_count = count($new_real_arr_orders);
        echo "<br> **** ~~~~ Дописались остальные заказы ($real_temp_count),  ++ ь ($count_order_art) *****<br>";
        break;
         }
 }
}
...
// *********************  формируем массив реальных заказов для 1С ******
 ...
 if (($priznzak_net_massivac== 0) AND ($priznzak_ne_ves_massiv==0)) {
    $arr_for_1C_file_temp[$key] = $arr_real_orders; // 

 } else {
    $arr_for_1C_file_temp[$key] = $arr_real_orders_error; // 
 ...
 ************************************************************************************************/
        echo "*****arr_for_1C_file_temp*********** МАССИВ ПО Которому формируем 1С файл *********<br>";
        echo "<pre>";
...
$file_json = "json_supply/$wb_path/".$Zakaz_v_1c." от ".date("Y-M-d").".json";
 ...
 ******************************************************************************************/
 ...
 $file_json_orders = "json_supply/$wb_path/".$Zakaz_v_1c." от ".date("Y-M-d")."_real_orders.json";
 $filedata_json_orders = json_encode($MEGA_arr_real_orders, JSON_UNESCAPED_UNICODE);
 file_put_contents($file_json_orders, $filedata_json_orders); // добавляем данные в файл с накопительным итогом
...
$link_q1=$path_arhives."/"."Stikers_".$Zakaz_v_1c." от ".date("Y-M-d").".zip";
 ...
 echo "<a href=\"$link_q1\">СКАЧАТЬ АРХИВ СО СТИКЕРАМИ И ФАЙЛАМ для 1С (старый и новый)</a>"; // 
...
<label for="wb">Номер заказа</label><br>
  ...
  <input hidden type="text" name="wb_path" value="$wb_path">
...
function error_jobs_by_get_stikers($token_wb, $supplyId) {
